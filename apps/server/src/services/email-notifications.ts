...
export function enqueuePublicationRemovalNotification(input: {
  ...
  const textBody = `${input.applicantName}，您好：\n\n您填报的“${input.itemName}”所属填报已由管理员从${term}第${input.week}周安排中删除。\n\n部门：${input.department}\n开始时间：${startTime}\n\n该份填报中的会议活动、原始填报及审核记录已一并删除，不再进入本周发布安排。\n\n${config.organizationName}周工作安排系统`;
  const htmlBody = `<div style="font-family:'Microsoft YaHei',Arial,sans-serif;color:#1e293b;line-height:1.8;max-width:640px;margin:auto"><h2 style="color:#8a1c22">周工作安排填报删除通知</h2><p>${escapeHtml(input.applicantName)}，您好：</p><p>您填报的 <strong>${escapeHtml(input.itemName)}</strong> 所属填报已由管理员从 <strong>${escapeHtml(term)}第${input.week}周</strong> 安排中删除。</p><div style="background:#fff7ed;border-left:4px solid #c2410c;padding:12px 16px;margin:20px 0"><strong>部门：</strong>${escapeHtml(input.department)}<br><strong>开始时间：</strong>${escapeHtml(startTime)}</div><p>该份填报中的会议活动、原始填报及审核记录已一并删除，不再进入本周发布安排。</p><p style="margin-top:28px;color:#64748b;font-size:13px">此邮件由${escapeHtml(config.organizationName)}周工作安排系统自动发送，请勿直接回复。</p></div>`;
  for (const recipient of input.recipients) {
    queueEmail({
